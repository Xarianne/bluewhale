...
- **Image names differ per recipe** (`bluewhale` / `bluewhale-noctalia`), both publishing plain `:latest` (+ the auto-generated `:44` and timestamp tags). Distinct names matter: two recipes sharing one GHCR package would race to publish the version tag (`:44`), and the last runner to finish would win.
