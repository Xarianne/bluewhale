...
- **Two image variants are built from `main`** via the workflow matrix: `recipe.yml` (GNOME + Hyprland/DMS, tagged `:main`) and `recipe-noctalia.yml` (Noctalia + Umbriel + Greeter, tagged `:noctalia`). Tags come from each recipe's `alt-tags` — required because Branch-based tags (`br-*-44`) can't produce bare tags, and only the default branch runs the daily cron.
- **Noctalia Greeter (greetd) is the login screen in the `:noctalia` variant, not GDM**: `gdm.service` is disabled and `greetd.service` enabled (both alias `display-manager.service`). GNOME stays installed and selectable, but GNOME-from-greetd is upstream "best-effort". Details: [noctalia.md](noctalia.md). The greetd/umbriel defaults under `files/system/` land in the DMS image too, but are inert there (neither package is installed in that variant).
